...
describe("deploy workflow database handling", () => {
  ...
  it("packages the single store-ready extension with no channel or signing inputs", () => {
    expect(workflow).toContain("node scripts/package-extension.mjs");
    // The CRX pipeline is gone: no per-channel overlay, no signing key, no
    // updates.xml base URL. The Chrome Web Store is the only distribution path.
    expect(workflow).not.toContain("EXTENSION_ENV");
    expect(workflow).not.toContain("EXTENSION_PEM_PATH");
    expect(workflow).not.toContain("EXTENSION_BASE_URL");
    expect(workflow).not.toContain("EXTENSION_CHANNEL");
  });
});
